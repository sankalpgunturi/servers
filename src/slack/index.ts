...
class SlackClient {
  ...
  private headers: { 'Auth-Token': string; 'Content-Type': string };

  constructor(runtimeToken: string, proxyPath: string) {
    this.proxyUrl = `https://proxy-stage.pinkfish.ai/request/slack/${proxyPath}`;
    this.headers = {
      'Auth-Token': `Bearer ${runtimeToken}`,
  ...
  async getChannels(limit: number = 100, cursor?: string): Promise<any> {
    ...
    if (!predefinedChannelIds) {
      const url = `${this.proxyUrl}/conversations.list`;
    ...
    for (const channelId of predefinedChannelIdsArray) {
      const url = `${this.proxyUrl}/conversations.info`;
      const params = new URLSearchParams({
        channel: channelId,
      });
  ...
  async postMessage(channel_id: string, text: string): Promise<any> {
    const url = `${this.proxyUrl}/chat.postMessage`;
  ...
  async addReaction(
    ...
    const url = `${this.proxyUrl}/reactions.add`;
    const response = await fetch(url, {
      method: 'POST',
      headers: this.headers,
      body: JSON.stringify({
  ...
  async getChannelHistory(
    ...
    const url = `${this.proxyUrl}/conversations.history`;
    const params = new URLSearchParams({
      channel: channel_id,
  ...
  async getThreadReplies(channel_id: string, thread_ts: string): Promise<any> {
    const url = `${this.proxyUrl}/conversations.replies`;
    const params = new URLSearchParams({
      channel: channel_id,
      ts: thread_ts,
    });
  ...
  async getUsers(limit: number = 100, cursor?: string): Promise<any> {
    const url = `${this.proxyUrl}/users.list`;
    const params = new URLSearchParams({
      limit: Math.min(limit, 200).toString(),
    });
  ...
  async getUserProfile(user_id: string): Promise<any> {
    const url = `${this.proxyUrl}/users.profile.get`;
    const params = new URLSearchParams({
      user: user_id,
      include_labels: 'true',
    });
...
async function main() {
  ...
  const proxyPath =
    process.env.PC_CONNECTION_ID ||
    process.env.PROXY_PATH ||
    'nY3fROfg0QItFlaXBusr';

  if (!runtimeToken) {
    console.error('Please set RUNTIME_TOKEN environment variable');
    process.exit(1);
  }
  ...
  const slackClient = new SlackClient(runtimeToken, proxyPath);
