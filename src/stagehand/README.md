...
```bash
npx -y @sankalpgunturi/server-stagehand
```

Or install it globally:

```bash
npm install -g @sankalpgunturi/server-stagehand
```
...
```json
{
  "mcpServers": {
    "stagehand": {
      "command": "npx",
      "args": ["-y", "@sankalpgunturi/server-stagehand"],
      "env": {
...
For running locally, first [**open Chrome in debug mode**](https://docs.stagehand.dev/examples/customize_browser#use-your-personal-browser) like so:
